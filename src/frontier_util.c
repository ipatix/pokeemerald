...
void ClearRankingHallRecords(void)
{
    ...
    for (i = 0; i < HALL_FACILITIES_COUNT; i++)
    {
        for (j = 0; j < 2; j++)
        {
            for (k = 0; k < 3; k++)
            {
                CopyTrainerId(gSaveBlock2Ptr->hallRecords1P[i][j][k].id, arr); // BUG: Passing 0 as a pointer instead of a pointer holding a value of 0.
    ...
    for (j = 0; j < 2; j++)
    {
        for (k = 0; k < 3; k++)
        {
            CopyTrainerId(gSaveBlock2Ptr->hallRecords2P[j][k].id1, arr); // BUG: Passing 0 as a pointer instead of a pointer holding a value of 0.
            CopyTrainerId(gSaveBlock2Ptr->hallRecords2P[j][k].id2, arr); // BUG: Passing 0 as a pointer instead of a pointer holding a value of 0.
